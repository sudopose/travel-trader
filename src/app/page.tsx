...
export default function Home() {
  ...
  const handleTravel = (locationId: string) => {
    ...
    if ("error" in result) {
      ...
      showNotification(
        "success",
        `Traveled to ${getLocationById(locationId)?.name}!`,
      );
    }
  };
  ...
  const handleUnlock = (locationId: string) => {
    ...
    if ("error" in result) {
      ...
      showNotification(
        "success",
        `Unlocked ${getLocationById(locationId)?.name}!`,
      );
    }
  };
  ...
  const totalItems = Object.values(gameState.inventory).reduce(
    (sum, count) => sum + count,
    0,
  );
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-950 via-blue-950 to-slate-950 p-4 md:p-8">
                  ...
                  .map(([goodId, count], index) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="capitalize text-white text-sm md:text-base">
                          {goodId}
                        </div>
      ...
      <div className="text-center text-slate-500 text-xs py-4">
        <p>
          💡 Tip: Discuss strategies together! Buy low, sell high, and watch for
          market events!
        </p>
      </div>
    </main>
